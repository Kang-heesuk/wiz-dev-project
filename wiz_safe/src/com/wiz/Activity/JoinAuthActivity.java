    ...
    public void onCreate(Bundle savedInstanceState) {
  	...
  	class callAuthCompleteApiThread extends Thread{
  		public void run(){
  			try{
  				...
  				String url = "http://www.heream.com/api/authComplete.jsp?ctn=" + URLEncoder.encode(enc_ctn) + "&authNum=" + URLEncoder.encode(enc_authNum);
  	...
  	class callAuthSMSApiThread extends Thread{
  		public void run(){
  			try{
  				String enc_ctn = WizSafeSeed.seedEnc(WizSafeUtil.getCtn(JoinAuthActivity.this));
  				String url = "http://www.heream.com/api/sendAuthSMS.jsp?ctn="+ URLEncoder.encode(enc_ctn);
  				HttpURLConnection urlConn = (HttpURLConnection) new URL(url).openConnection();
  				BufferedReader br = new BufferedReader(new InputStreamReader(urlConn.getInputStream(),"euc-kr"));	
  				String temp;
  				returnXML = new ArrayList<String>();
