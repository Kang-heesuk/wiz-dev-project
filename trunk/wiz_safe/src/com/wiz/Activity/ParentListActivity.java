  	...
  	class CallGetParentListApiThread extends Thread{
  		public void run(){
  			try{
  				...
  				String url = "http://www.heream.com/api/getParentList.jsp?ctn="+ URLEncoder.encode(enc_ctn);
  	...
  	class CallDeleteApiThread extends Thread{
  		public void run(){
  			try{
  				...
  				String url = "http://www.heream.com/api/deleteRelation.jsp?parentCtn="+ URLEncoder.encode(enc_selectedCtn) + "&childCtn=" + URLEncoder.encode(enc_ctn) + "&me=child";
  				HttpURLConnection urlConn = (HttpURLConnection) new URL(url).openConnection();
  				BufferedReader br = new BufferedReader(new InputStreamReader(urlConn.getInputStream(),"euc-kr"));	
  				String temp;
  				ArrayList<String> returnXML = new ArrayList<String>();
